...
class AdminObgyn extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\Obgyn')
            ->first();
        $this->page_title = "Obgyn";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = Obgyn::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = Obgyn::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => Obgyn::class,
            ]);
            Obgyn::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = Obgyn::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = Obgyn::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-obgyn')->layout('admin.layouts.master');
    }
}
