...
class AdminAnak extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\Anak')
            ->first();
        $this->page_title = "Anak";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = Anak::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = Anak::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => Anak::class,
            ]);
            Anak::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = Anak::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = Anak::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-anak')->layout('admin.layouts.master');
    }
}
