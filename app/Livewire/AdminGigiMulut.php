...
class AdminGigiMulut extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\GigiMulut')
            ->first();
        $this->page_title = "GigiMulut";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = GigiMulut::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = GigiMulut::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => GigiMulut::class,
            ]);
            GigiMulut::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = GigiMulut::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = GigiMulut::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-gigi-mulut')->layout('admin.layouts.master');
    }
}
