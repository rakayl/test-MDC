...
class AdminSaraf extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\Saraf')
            ->first();
        $this->page_title = "Saraf";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = Saraf::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = Saraf::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => Saraf::class,
            ]);
            Saraf::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = Saraf::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = Saraf::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-saraf')->layout('admin.layouts.master');
    }
}
