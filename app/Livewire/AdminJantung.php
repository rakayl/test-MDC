...
class AdminJantung extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\Jantung')
            ->first();
        $this->page_title = "Jantung";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = Jantung::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = Jantung::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => Jantung::class,
            ]);
            Jantung::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = Jantung::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = Jantung::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-jantung')->layout('admin.layouts.master');
    }
}
