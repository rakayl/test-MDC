...
class AdminPenyakitDalam extends Component
{
    public $activeQueue;
    ...
    public function mount()
    {
        $this->activeQueue = Ticket::with(['tiketable','tiketable.user'])
            ...
            ->where('tiketable_type','App\Models\PenyakitDalam')
            ->first();
        $this->page_title = "PenyakitDalam";
        
    }
    public function done()
    {
        if(isset($this->activeQueue)){
             $update = PenyakitDalam::where('id', $this->activeQueue->tiketable_id)
...
//        $this->no = 1;
//            
        $queues = PenyakitDalam::whereDate('queue_date', today())
            ->where('status','waiting')
            ->orderBy('number')
            ->get();
        ...
        if (empty($this->activeQueue)){
            ...
            $create = Ticket::create([
                ...
                'tiketable_type' => PenyakitDalam::class,
            ]);
            PenyakitDalam::where('id', $queues->id)
                    ->update([
...
//                    ->where('status','waiting')
        ...
        $this->waitingQueues = PenyakitDalam::whereDate('queue_date', today())
    ...
    public function render()
    {
        $this->waitingQueues = PenyakitDalam::whereDate('queue_date', today())
            ->where('status','waiting')->get();
        return view('livewire.admin-penyakit-dalam')->layout('admin.layouts.master');
    }
}
