...
function get_amount($amount, $currency = null, $precision = null)
{
    if (!is_numeric($amount)) return "Not Number";
        $amount = ($precision) ? number_format($amount, $precision, ".", ".") : number_format($amount, 0, ".", ".");
...
// helpers.php or any custom helper file
if (! function_exists('get_profile_image')) {
    function get_profile_image($image, $directory, $defaultImage)
    {
        if ($image) {
            return asset('storage/' . $directory . '/' . $image);
        }

        // Return default image path if no image is available
        return asset($defaultImage);
    }
}

// helpers.php or any custom helper file
if (! function_exists('get_document_driver')) {
    function get_document_driver($image, $directory)
    {
        if ($image) {
            return asset('storage/' . $directory . '/' . $image);
        }
